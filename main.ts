...
class Data {
    ...
    constructor(element: string, api: string) {
    ...
    checkLocalStorage() {
        const locStorageCurrent = localStorage.getItem("value")
    ...
    // set slider autoplay ------------------
    autoPlayFunc(leftBtn: HTMLButtonElement, rightBtn: HTMLButtonElement) {
    ...
    fadeToRight(leftBtn, rightBtn) {
        ...
        const allSelectedDots = document.querySelectorAll(".navigation-dot") as NodeListOf<HTMLDivElement>;

        allSelectedDots.forEach((element) => {
            const elementDataSet = element.dataset.set;

            if (elementDataSet) {
                if (this.current === parseInt(elementDataSet)) {
    ...
    showSlidesByDots(index) {
        ...
        const allSelectedDots = document.querySelectorAll(".slide") as NodeListOf<HTMLDivElement>;

        allSelectedDots.forEach(element => {

            const elementDataSet = element.dataset.id;
    ...
    changingDotsActivity(leftBtn, rightBtn) {
        ...
        allSelectedDots.forEach((element, index) => {
            const elementDataSet = element.dataset.set;
            ...
            element.addEventListener("click", (event: Event) => {
                ...
                if (eventTarget.dataset.set) {
                    this.current = parseInt(eventTarget.dataset.set)
                    this.handleButtons(leftBtn, rightBtn)
...
let slider1: MyData = new Data(".slider", 'http://localhost:3000/data')
slider1.init()
